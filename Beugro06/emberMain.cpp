...
void test_1(int ma) {
    TEST(test1, kiir)  // lehetne finomabban a teszteket
      ...
      EXPECT_STR_EQ("Hallgato", en.foglalkozas());
...
void test_3(int ma) {
    TEST(test3, kiir)
      ...
      EXPECT_STR_EQ("TUDOR6", valaki.getNeptun()); // lemásolódott a neptun is? Ki másolta le?
      Ember *okos = &valaki;
      kiirEllp(okos, ma, "Okos Toni", "Hallgato", 1970); // kiírja és ellenőrzi az adatokat

      // Azt várjuk, hogy kivételt dobjon
      EXPECT_THROW(cout << okos->getKor(1900), const char*);
...
void test_4(int ma) {
    TEST(test4, kkir)
      ...
      EXPECT_STR_EQ("BT1234", en.getNeptun()); // lemásolódott a neptun is? Ki másolta le?
      ...
      EXPECT_STR_EQ("TUDOR6", toni.getNeptun()); // neptun is jó?
